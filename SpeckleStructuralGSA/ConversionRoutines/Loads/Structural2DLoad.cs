...
namespace SpeckleStructuralGSA
{
  ...
  public class GSA2DLoad : GSABase<Structural2DLoad>
  {
    ...
    public string SetGWACommand()
    {
      ...
      if (Initialiser.Instance.Settings.TargetLayer == GSATargetLayer.Analysis)
      {
        elementRefs = Initialiser.Instance.Cache.LookupIndices(typeof(GSA2DElement).GetGSAKeyword(), load.ElementRefs).Where(x => x.HasValue).Select(x => x.Value).ToList();
        groupRefs = Initialiser.Instance.Cache.LookupIndices(typeof(GSA2DElementMesh).GetGSAKeyword(), load.ElementRefs).Where(x => x.HasValue).Select(x => x.Value).ToList();
      }
      else if (Initialiser.Instance.Settings.TargetLayer == GSATargetLayer.Design)
      {
        elementRefs = new List<int>();
        groupRefs = Initialiser.Instance.Cache.LookupIndices(typeof(GSA2DMember).GetGSAKeyword(), load.ElementRefs).Where(x => x.HasValue).Select(x => x.Value).ToList();
      }
      else
      {
        return "";
  ...
  public static partial class Conversions
  {
    ...
    public static SpeckleObject ToSpeckle(this GSA2DLoad dummyObject)
    {
      ...
      var elements = Initialiser.Instance.GSASenderObjects.Get<GSA2DElement>();
      var members = Initialiser.Instance.GSASenderObjects.Get<GSA2DMember>();
