...
namespace SpeckleStructuralGSA
{
  ...
  public class GSA1DLoad : IGSASpeckleContainer
  {
    ...
    public void SetGWACommand()
    {
      ...
      if (Initialiser.Settings.TargetLayer == GSATargetLayer.Analysis)
      {
        elementRefs = Initialiser.Indexer.LookupIndices(typeof(GSA1DElement).GetGSAKeyword(), typeof(GSA1DElement).Name, load.ElementRefs).Where(x => x.HasValue).Select(x => x.Value).ToList();
        groupRefs = Initialiser.Indexer.LookupIndices(typeof(GSA1DElementPolyline).GetGSAKeyword(), typeof(GSA1DElementPolyline).Name, load.ElementRefs).Where(x => x.HasValue).Select(x => x.Value).ToList();
      }
      else if (Initialiser.Settings.TargetLayer == GSATargetLayer.Design)
      {
        elementRefs = new List<int>();
        groupRefs = Initialiser.Indexer.LookupIndices(typeof(GSA1DMember).GetGSAKeyword(), typeof(GSA1DMember).Name, load.ElementRefs).Where(x => x.HasValue).Select(x => x.Value).ToList();
        groupRefs.AddRange(Initialiser.Indexer.LookupIndices(typeof(GSA1DElementPolyline).GetGSAKeyword(), typeof(GSA1DElementPolyline).Name, load.ElementRefs).Where(x => x.HasValue).Select(x => x.Value).ToList());
      }
      else
      {
        return;
  ...
  public static partial class Conversions
  {
    ...
    {
      new GSA1DLoad() { Value = load }.SetGWACommand();
    ...
    {
      if (!Initialiser.GSASenderObjects.ContainsKey(typeof(GSA1DLoad)))
        Initialiser.GSASenderObjects[typeof(GSA1DLoad)] = new List<object>();

      List<GSA1DLoad> loads = new List<GSA1DLoad>();
      List<GSA1DElement> elements = Initialiser.Settings.TargetLayer == GSATargetLayer.Analysis ? Initialiser.GSASenderObjects[typeof(GSA1DElement)].Cast<GSA1DElement>().ToList() : new List<GSA1DElement>();
      List<GSA1DMember> members = Initialiser.Settings.TargetLayer == GSATargetLayer.Design ? Initialiser.GSASenderObjects[typeof(GSA1DMember)].Cast<GSA1DMember>().ToList() : new List<GSA1DMember>();
      ...
      Initialiser.GSASenderObjects[typeof(GSA1DLoad)].RemoveAll(l => deletedLines.Contains((l as IGSASpeckleContainer).GWACommand));
      foreach (var kvp in Initialiser.GSASenderObjects)
        kvp.Value.RemoveAll(l => (l as IGSASpeckleContainer).SubGWACommand.Any(x => deletedLines.Contains(x)));

      // Filter only new lines
      string[] prevLines = Initialiser.GSASenderObjects[typeof(GSA1DLoad)].Select(l => (l as IGSASpeckleContainer).GWACommand).ToArray();
      ...
      foreach (string p in newLines)
      {
        ...
        if (Initialiser.Settings.TargetLayer == GSATargetLayer.Analysis)
        {
          ...
          foreach (string nRef in initLoad.Value.ElementRefs)
          {
            ...
            GSA1DLoad match = loadSubList.Count() > 0 ? loadSubList.Where(l => (l.Value.Loading.Value as List<double>).SequenceEqual(load.Value.Loading.Value as List<double>)).First() : null;
            if (match != null)
