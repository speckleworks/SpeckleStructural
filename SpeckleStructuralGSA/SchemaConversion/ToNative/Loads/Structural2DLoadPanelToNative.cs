    ...
    public static string ToNative(this Structural2DLoadPanel loadPanel)
    {
      ...
      return Helper.ToNativeTryCatch(loadPanel, () =>
      {
        ...
        if (string.IsNullOrEmpty(loadPanel.LoadPlaneRef))
        {
          ...
          {
            Initialiser.AppResources.Messenger.Message(MessageIntent.Display, MessageLevel.Error, "Generating axis from coordinates for 2D load panel", loadPanel.ApplicationId);
          }
        }
        else
        {
            ...
            if (gsaGridSurface.PlaneRefType == GridPlaneAxisRefType.Reference && gsaGridSurface.PlaneIndex.ValidNonZero())
            {
              ...
              if (gsaGridPlane.FromGwa(gsaGridPlaneGwa))
              {
                if (gsaGridPlane.AxisRefType == GridPlaneAxisRefType.Reference && gsaGridPlane.AxisIndex.ValidNonZero())
                {
                  ...
                  {
                    Initialiser.AppResources.Messenger.Message(MessageIntent.Display, MessageLevel.Error, "Unable to parse AXIS GWA", loadPanel.ApplicationId);
                  }
                }
                else
                {
                  Initialiser.AppResources.Messenger.Message(MessageIntent.Display, MessageLevel.Error, "Invalid AXIS reference", loadPanel.ApplicationId);
                }
              }
              else
              {
                Initialiser.AppResources.Messenger.Message(MessageIntent.Display, MessageLevel.Error, "Unable to parse GRID_PLANE GWA", loadPanel.ApplicationId);
              }
            }
            else
            {
              Initialiser.AppResources.Messenger.Message(MessageIntent.Display, MessageLevel.Error, "Invalid GRID_PLANE reference", loadPanel.ApplicationId);
            }
          }
          else
          {
            Initialiser.AppResources.Messenger.Message(MessageIntent.Display, MessageLevel.Error, "Unable to parse GRID_SURFACE GWA", loadPanel.ApplicationId);
          }
        }
